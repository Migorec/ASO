...
import Data.List (zipWith4, zipWith7, intersperse)
import Text.Printf
import System.Cmd
...
lab01 p n = do 
               ...
               _ <- system  $ "(echo 'set x2tics' && echo 'set x2range [  " ++ show (-fn*p/sqrt(fn*p*q))++":" ++ show ((fn-fn*p)/sqrt(fn*p*q))++ "]' && echo 'set label \"p=" ++ show p ++ "\\\\nq="++show q++"\\\\nn="++show n ++"\" at screen 0.2,0.7' && cat plot.gp) | gnuplot && ps2pdf -dEPSCrop plot.eps plot.pdf && pdflatex report.tex"       
               return ()
    where q = 1 - p
...
--          pBs = map fromBigFloat bpBs :: [Double]
          ...
          toFile = zipWith4 (\k xi pB pG -> show k ++ "\t" ++ 
                                            show xi ++ "\t" ++
                                            show pB ++ "\t" ++
                                            show pG) ks xis pBs pGs
